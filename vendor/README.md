...
`tetravox-module-sdk-1-0.3.5.tgz` — the module SDK this repository builds against
(host API 1, core 0.3.5), the official release tarball, replacing the pre-release pin taken from
Tetravox PR #18. It carries `scene.sampleVolume(datasetId, worldPoints, { order })` — §4.3's third
read shape beside the probe and the bounded box, capped at 2,000,000 points — which is what makes
the 0.25 mm template slide in `src/modelsnap.ts` one batch read instead of thousands of round trips.
